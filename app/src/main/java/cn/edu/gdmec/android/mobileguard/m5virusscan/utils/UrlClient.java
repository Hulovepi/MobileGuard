...
public class UrlClient {
    public static String UrlPost(String url,String content){
        try {
            ...
            //设置消息的类型
            mHttpURLConnection.setRequestProperty("ContentType","application/json;charset=UTF-8");
            //接受输入流
            ...
            //输出流，用来发送请求，http请求实际上直到这个函数里面菜正式发送出去
            OutputStream out = mHttpURLConnection.getOutputStream();
            //创建字符流对象并用高效缓冲流包装它，使获得最高的效率，发送的是字符串推荐用字符流，其他数据就用字节流
